...
def test_changelog_from_subdirectory_with_custom_template_and_output_dir(
    ...
    assert not default_changelog.exists(), (
        "Changelog should NOT be written to repo root when output_dir is configured"
    )
    assert not pkg_changelog.exists(), (
        "Changelog should NOT be written to package subdirectory when output_dir is configured"
    )
